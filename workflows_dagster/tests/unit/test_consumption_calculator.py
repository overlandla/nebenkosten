...
class TestConsumptionCalculator:
    ...
    def test_calculate_annual_consumption_interpolated(self, calculator):
        ...
        readings = pd.DataFrame({
            'timestamp': pd.date_range('2024-01-15', periods=12, freq='M', tz='UTC'),
    ...
    def test_combine_meter_readings_basic(self, calculator):
        ...
        old_readings = pd.DataFrame({
            'timestamp': pd.date_range('2024-01-01', '2024-06-15', freq='D', tz='UTC'),
            'value': [100 + i * 0.5 for i in range(166)]
        })

        new_readings = pd.DataFrame({
            'timestamp': pd.date_range('2024-06-15', '2024-12-31', freq='D', tz='UTC'),
    ...
    def test_combine_meter_readings_empty_old(self, calculator):
        ...
        old_readings = pd.DataFrame()
        new_readings = pd.DataFrame({
            'timestamp': pd.date_range('2024-06-15', periods=10, freq='D', tz='UTC'),
            'value': range(10)
        })
    ...
    def test_combine_meter_readings_no_overlap(self, calculator):
        ...
        old_readings = pd.DataFrame({
            'timestamp': pd.date_range('2024-01-01', '2024-06-10', freq='D', tz='UTC'),
            'value': range(100, 262)
        })

        new_readings = pd.DataFrame({
            'timestamp': pd.date_range('2024-06-20', '2024-12-31', freq='D', tz='UTC'),
            'value': range(0, 195)
        })
