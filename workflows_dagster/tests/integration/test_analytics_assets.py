...
""")
    ...
    def test_meter_discovery_integration(self, mock_influx_official, mock_influx_resource, mock_config_resource):
        ...
        with patch('src.influx_client.InfluxClient.discover_available_meters') as mock_discover:
            mock_discover.return_value = ['test_meter_1', 'test_meter_2']

            result = meter_discovery(context, mock_influx_resource, mock_config_resource)
    ...
    @patch('src.influx_client.InfluxClient_Official')
    def test_raw_meter_data_integration(self, mock_influx_official, mock_influx_resource, mock_config_resource):
        ...
        with patch('src.influx_client.InfluxClient.fetch_all_meter_data') as mock_fetch:
            ...
            result = raw_meter_data(context, meters, mock_influx_resource, mock_config_resource)
    ...
    @patch('src.influx_client.InfluxClient_Official')
    def test_interpolated_meter_series_integration(self, mock_influx_official, mock_influx_resource, mock_config_resource):
        ...
        raw_data = {
            'test_meter': pd.DataFrame({
                'timestamp': pd.date_range('2024-01-01', '2024-01-31', freq='5D', tz='UTC'),
...
""")
        ...
        # Mock data flow
        with patch('src.influx_client.InfluxClient.discover_available_meters') as mock_discover, \
             patch('src.influx_client.InfluxClient.fetch_all_meter_data') as mock_fetch:
            ...
            mock_fetch.return_value = pd.DataFrame({
                'timestamp': pd.date_range('2024-01-01', '2024-01-31', freq='2D', tz='UTC'),
                'value': range(100, 116)
            })
            
            raw_data_result = raw_meter_data(context, meters, influx_resource, config_resource)
