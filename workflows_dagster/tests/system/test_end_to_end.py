...
class TestTibberSyncE2E:
    ...
    def test_complete_tibber_sync_flow(
        ...
        mock_response.json.return_value = {
            "data": {"viewer": {"homes": [{"consumption": {"nodes": tibber_data}}]}}
        }
...
class TestAnalyticsE2E:
    ...
    @patch(
        "workflows_dagster.dagster_project.assets.influxdb_writer_assets.InfluxDBResource.get_client"
    )
    def test_complete_analytics_pipeline(
        self, mock_get_client, mock_calc_class, mock_processor_class, mock_influx_class
    ):
        ...
        mock_influx.discover_available_meters.return_value = [
            "strom_total",
            "gas_total",
        ]
        mock_influx.fetch_all_meter_data.side_effect = (
            lambda meter_id, start: generate_multi_meter_data([meter_id], days=31)[
                meter_id
            ]
        )
...
class TestDataQuality:
    ...
    def test_interpolation_produces_valid_data(
        ...
        raw_data_output = fetch_meter_data(
            context, ["meter1"], mock_influxdb_resource, mock_config_resource
        )

        # Interpolate
        daily, monthly = interpolated_meter_series(
            context, raw_data_output.value, mock_influxdb_resource, mock_config_resource
        )
    ...
    def test_consumption_never_exceeds_reasonable_bounds(self):
        ...
        context = build_asset_context()
        daily_series = {
            "meter1": generate_meter_readings(days=31, daily_increment=10.0)
        }
        master_series = {}

        with patch(
            "dagster_project.assets.analytics_assets.ConsumptionCalculator"
        ) as mock_calc_class:
            from tests.fixtures.mock_data import generate_consumption_data
