...
# Function to prompt for credentials (available in all modes)
prompt_credentials() {
    local secrets_dir=$1
    local update_mode=$2

    echo -e "\n${BL}═══════════════════════════════════════════════════════${CL}"
    echo -e "${BL}  Configuration Setup${CL}"
    echo -e "${BL}═══════════════════════════════════════════════════════${CL}\n"

    # Check if secrets already exist in update mode
    if [ "$update_mode" = "true" ] && [ -f "$secrets_dir/influxdb.env" ]; then
        echo -e "${YW}Existing configuration detected.${CL}\n"
        read -p "Do you want to update credentials? (y/N): " update_creds
        if [[ ! "$update_creds" =~ ^[Yy]$ ]]; then
            msg_ok "Keeping existing credentials"
            return 0
        fi
    fi

    # InfluxDB Configuration
    echo -e "${BL}InfluxDB Configuration:${CL}"
    echo -e "${YW}Enter your InfluxDB connection details.${CL}"
    echo -e "${YW}(Press Enter to use defaults shown in brackets)${CL}\n"

    read -p "InfluxDB URL [http://192.168.1.75:8086]: " influx_url
    influx_url=${influx_url:-http://192.168.1.75:8086}

    read -p "InfluxDB Token: " influx_token
    while [ -z "$influx_token" ]; do
        echo -e "${RD}Token cannot be empty!${CL}"
        read -p "InfluxDB Token: " influx_token
    done

    read -p "InfluxDB Organization: " influx_org
    while [ -z "$influx_org" ]; do
        echo -e "${RD}Organization cannot be empty!${CL}"
        read -p "InfluxDB Organization: " influx_org
    done

    read -p "Raw data bucket [lampfi]: " influx_bucket_raw
    influx_bucket_raw=${influx_bucket_raw:-lampfi}

    read -p "Processed data bucket [lampfi_processed]: " influx_bucket_processed
    influx_bucket_processed=${influx_bucket_processed:-lampfi_processed}

    # Create InfluxDB env file
    cat > "$secrets_dir/influxdb.env" <<EOF
# InfluxDB Configuration
# Auto-generated by Dagster installation script
...
EOF
    chmod 600 "$secrets_dir/influxdb.env"
    msg_ok "Created influxdb.env with your credentials"

    # Tibber Configuration (optional)
    echo -e "\n${BL}Tibber Configuration (Optional):${CL}"
    echo -e "${YW}Tibber integration is optional. Skip if you don't use Tibber.${CL}\n"
    read -p "Configure Tibber? (y/N): " configure_tibber

    if [[ "$configure_tibber" =~ ^[Yy]$ ]]; then
        read -p "Tibber API Token: " tibber_token
        read -p "Tibber API URL [https://api.tibber.com/v1-beta/gql]: " tibber_url
        tibber_url=${tibber_url:-https://api.tibber.com/v1-beta/gql}

        cat > "$secrets_dir/tibber.env" <<EOF
# Tibber API Configuration
# Auto-generated by Dagster installation script

# Tibber API Token
TIBBER_API_TOKEN=$tibber_token

# Tibber API URL
TIBBER_API_URL=$tibber_url
EOF
        chmod 600 "$secrets_dir/tibber.env"
        msg_ok "Created tibber.env with your credentials"
    else
        # Create placeholder
        cat > "$secrets_dir/tibber.env" <<EOF
# Tibber API Configuration
# Optional - only needed for Tibber sync
...
EOF
        chmod 600 "$secrets_dir/tibber.env"
        msg_ok "Created tibber.env placeholder"
    fi

    echo -e "\n${GN}✓ Configuration complete!${CL}\n"
}
...
if [ -n "$FUNCTIONS_FILE_PATH" ]; then
    ...
    if [ -d /opt/dagster-workflows/nebenkosten/.git ]; then
      ...
      msg_info "Ensuring required directories exist"
      cd /opt/dagster-workflows/nebenkosten
      mkdir -p logs storage secrets config
      ...
      # Prompt for credentials (will skip if user chooses to keep existing)
      prompt_credentials "/opt/dagster-workflows/nebenkosten/secrets" "true"

      msg_info "Fixing ownership"
      chown -R dagster:dagster /opt/dagster-workflows
      msg_ok "Ownership fixed"
      ...
      echo -e "${GN}✓ All services are running${CL}"
      echo -e "${GN}✓ Configuration database updated${CL}"
      echo -e "${GN}✓ Credentials configured${CL}\n"

      echo -e "${YW}📋 Useful Commands:${CL}"
      echo -e "  ${BL}systemctl status dagster-*${CL}         - Check service status"
      echo -e "  ${BL}journalctl -u dagster-daemon -f${CL}    - View logs"
      echo -e "  ${BL}configure-dagster${CL}                  - Update configuration\n"
...
# Prompt for credentials (fresh install)
prompt_credentials "$REPO_DIR/secrets" "false"
...
echo -e "${BL}1.${CL} Enable schedules in Dagster UI:"
echo -e "   - Navigate to ${GN}http://$IP_ADDR:3000${CL}"
echo -e "   - Go to Automation → Schedules"
echo -e "   - Enable 'analytics_daily' and optionally 'tibber_sync_hourly'\n"

echo -e "${BL}2.${CL} Monitor your workflows:"
echo -e "   - View runs in the Dagster UI"
echo -e "   - Check logs with: ${GN}journalctl -u dagster-* -f${CL}\n"

echo -e "${YW}📝 Configuration:${CL}"
echo -e "  ${BL}InfluxDB config:${CL} $INSTALL_DIR/nebenkosten/secrets/influxdb.env"
echo -e "  ${BL}Tibber config:${CL} $INSTALL_DIR/nebenkosten/secrets/tibber.env"
echo -e "  ${BL}Update config:${CL} Run ${GN}configure-dagster${CL} or edit files manually\n"
